...
func TestSystemGroupMaterialization(t *testing.T) {
	t.Run("system groups are written to OpenFGA on first request", func(t *testing.T) {
		...
		mockFGA := &mockFGAClient{
			WriteFunc: func(ctx context.Context, in *openfgav1.WriteRequest, opts ...grpc.CallOption) (*openfgav1.WriteResponse, error) {
				writeCalled = true
				if in.Writes != nil {
					writtenTuples = append(writtenTuples, in.Writes.TupleKeys...)
				}
				return &openfgav1.WriteResponse{}, nil
			},
			CheckFunc: func(ctx context.Context, in *openfgav1.CheckRequest, opts ...grpc.CallOption) (*openfgav1.CheckResponse, error) {
		...
		auth := &webhook.SubjectAccessReviewAuthorizer{
			...
			ProtectedResourceCache:  prCache,
			DiscoveryClient:         &mockDiscoveryClient{},
			SystemGroupMaterializer: webhook.NewSystemGroupMaterializerForTest(mockFGA, "test-store"),
		}
		...
		attributes := &mockAttributes{
			...
			user: &user.DefaultInfo{
				...
				Extra: map[string][]string{
					iamv1alpha1.ParentAPIGroupExtraKey: {"resourcemanager.miloapis.com"},
					iamv1alpha1.ParentKindExtraKey:     {"Project"},
					iamv1alpha1.ParentNameExtraKey:     {"proj-1"},
				},
			},
		}
		...
		require.NoError(t, err)
		assert.Equal(t, authorizer.DecisionAllow, decision)
		assert.True(t, writeCalled, "Write should be called to persist system group memberships")
		assert.Len(t, writtenTuples, 2)

		// Verify the written tuples reference the correct user and groups
		for _, tuple := range writtenTuples {
			assert.Equal(t, "iam.miloapis.com/InternalUser:user-uid-1", tuple.User)
			assert.Equal(t, "member", tuple.Relation)
			assert.True(t,
				tuple.Object == "iam.miloapis.com/InternalUserGroup:system_authenticated" ||
					tuple.Object == "iam.miloapis.com/InternalUserGroup:system_masters",
				"unexpected group object: %s", tuple.Object,
			)
		}
	})

	t.Run("system groups are written only once per user UID", func(t *testing.T) {
		writeCallCount := 0
		...
		assert.Equal(t, 1, writeCallCount, "Write should be called exactly once for the same user UID")
	})

	t.Run("non-system groups are not written", func(t *testing.T) {
		writeCallCount := 0
		...
		attributes := &mockAttributes{
			...
			user: &user.DefaultInfo{
				Name:   "test-user",
				UID:    "user-uid-no-system",
				Groups: []string{"custom-group", "another-group"}, // No system: prefix
				Extra: map[string][]string{
					iamv1alpha1.ParentAPIGroupExtraKey: {"resourcemanager.miloapis.com"},
					iamv1alpha1.ParentKindExtraKey:     {"Project"},
					iamv1alpha1.ParentNameExtraKey:     {"proj-1"},
		...
		assert.Equal(t, 0, writeCallCount, "Write should not be called when there are no system groups")
	})
}
