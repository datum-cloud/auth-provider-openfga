...
type PolicyBindingUpdate struct {
	PolicyBinding *iammiloapiscomv1alpha1.PolicyBinding
	Subjects      []iammiloapiscomv1alpha1.Subject
}
...
func (f *GroupFinalizer) cleanupAssociatedResources(ctx context.Context, params UpdateResourcesParams) error {
	...
	case *iammiloapiscomv1alpha1.GroupMembershipList:
	...
	case *iammiloapiscomv1alpha1.PolicyBindingList:
		// Update all PolicyBindings that reference this group
		for _, pb := range v.Items {
			var updatedSubjects []iammiloapiscomv1alpha1.Subject
			hasGroupRef := false
...
func (f *GroupFinalizer) deleteGroupMemberships(ctx context.Context, group *iammiloapiscomv1alpha1.Group) error {
	return f.cleanupAssociatedResources(ctx, UpdateResourcesParams{
		Group:        group,
		List:         &iammiloapiscomv1alpha1.GroupMembershipList{},
		ResourceType: "GroupMembership",
	})
}

// deletePolicyBindings deletes all PolicyBinding objects associated with the given group
func (f *GroupFinalizer) updatePolicyBindings(ctx context.Context, group *iammiloapiscomv1alpha1.Group) error {
	return f.cleanupAssociatedResources(ctx, UpdateResourcesParams{
		Group:        group,
		List:         &iammiloapiscomv1alpha1.PolicyBindingList{},
...
func (r *GroupReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&iammiloapiscomv1alpha1.Group{}).
		Named("group").
		Complete(r)
}
